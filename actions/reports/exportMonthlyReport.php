...
foreach ($allWorkers as $wName) {
    ...
    $sheet->getColumnDimension($colString)->setWidth(15);
    $colIndex++;
}
...
$headerStyle = [
    ...
    'fill' => ['fillType' => Fill::FILL_SOLID, 'startColor' => ['rgb' => '4472C4']], // Azul solicitado
...
// Precio/Hora (Calculado)
$colP_T = Coordinate::stringFromColumnIndex($cIdx++);

// Suma Bill (Total $)
$colB_T = Coordinate::stringFromColumnIndex($cIdx++);
$sheet->setCellValue("{$colP_T}{$rowMins}", "");
$sheet->setCellValue("{$colB_T}{$rowMins}", "");

$sheet->setCellValue("{$colB_T}{$rowHours}", "=SUM({$colB_T}2:{$colB_T}" . ($rowMins - 1) . ")");
$sheet->setCellValue("{$colP_T}{$rowHours}", "=SUM({$colP_T}2:{$colP_T}" . ($rowMins - 1) . ")");
...
$sheet->getStyle("{$colP}2:{$colT}{$rowHours}")->getNumberFormat()->setFormatCode('€ #,##0.00');
