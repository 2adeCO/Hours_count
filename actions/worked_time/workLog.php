<?php
session_start();
require '../hours_count_connection.php';
...
<head>
    ...
    <title>Registro de Horas | HOURS COUNT</title>
    ...
    <style>
        ...
        .control-group label { font-size: 0.75rem; font-weight: 700; color: #000; text-transform: uppercase; }

        input, select { padding: 12px; border: 1.5px solid #e2e8f0; border-radius: 6px; outline: none; font-family: inherit; font-size: 0.95rem; }
        ...
        .status-message.show { display: block; background: #fff5f5; color: #c53030; border: 1px solid #feb2b2; border-left: 5px solid var(--danger); }
        ...
        .collapsible-content.show { 
            padding: 25px; max-height: 500px; border: 1px solid #e2e8f0; 
        ...
        .btn-submit-main:hover { background: #219150; transform: translateY(-2px); }
        ...
        td { padding: 1.25rem 1.5rem; border-bottom: 1px solid #edf2f7; font-size: 0.95rem; }

        /* ACCIONES RÁPIDAS */
        .btn-action {
            padding: 8px 12px; border: 1px solid #e2e8f0; border-radius: 4px; cursor: pointer;
        ...
        .editBtn { color: #92400e; }
        .editBtn:hover { background: #fffaf0; border-color: #f6ad55; }
        .add15Btn, .sub15Btn { color: #166534; }
        .add15Btn:hover, .sub15Btn:hover { background: #f0fdf4; border-color: #68d391; }
        .deleteBtn { color: var(--danger); border-color: #fed7d7; }
        .deleteBtn:hover { background: #fff5f5; border-color: var(--danger); }
        .addCommentBtn { color: var(--primary); border-color: #eef2ff; }
        .addCommentBtn:hover { background: #f0f0ff; border-color: var(--primary); }

        .btn-action:disabled { opacity: 0.4; cursor: not-allowed; }

        /* FILA COMENTARIOS */
        .comment-row td { background: #f8fafc; padding: 15px 30px; }
        .comment-box {
            background: white; border: 1px solid #e2e8f0; border-radius: 8px; padding: 15px;
            display: flex; gap: 10px; align-items: center;
        }
        .comment-input { flex-grow: 1; border: 1.5px solid #e2e8f0; padding: 10px; border-radius: 6px; font-family: inherit; }
        ...
        #mobileCommentArea { width: 100%; border: 1.5px solid #e2e8f0; padding: 12px; border-radius: 6px; font-family: inherit; margin-top: 15px; box-sizing: border-box; resize: vertical; min-height: 100px; }
        ...
        .dropdown-item { padding: 10px 12px; cursor: pointer; border-bottom: 1px solid #f1f5f9; font-size: 0.95rem; transition: background 0.1s; }
...
<body>
    ...
    <div class="bg-watermark">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 50 50" width="800" height="800">
            ...
            <path d="M 30 15 L 30 33 C 30 34 30.9 35 32 35 C 33.1 35 34 34 34 33 L 34 15 L 37 18 L 32 9 L 27 18 L 30 15 Z" fill="#402EB6"/>
        </svg>
    </div>
    ...
    <header>
        <div class="brand">
    <a href="../../index.php">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="5 5 40 40" width="44" height="44">
            <circle cx="25" cy="25" r="18" 
                    stroke="white" stroke-width="3" fill="none" 
                    stroke-dasharray="85 28" stroke-linecap="round" />
            
            <rect x="16" y="27" width="4" height="8" rx="1.5" fill="white" />
            <rect x="23" y="21" width="4" height="14" rx="1.5" fill="white" />
            
            <path d="M 30 15 L 30 33 C 30 34 30.9 35 32 35 C 33.1 35 34 34 34 33 L 34 15 L 37 18 L 32 9 L 27 18 L 30 15 Z" 
                  fill="white" />
        </svg>
    </a>
</div>
...
<div class="recent-shortcuts" style="margin-bottom: 15px; background: white; padding: 15px; border-radius: var(--radius); border: 1px dashed var(--primary);">
    ...
    <?php foreach ($recentServices as $recent): ?>
        <button type="button" 
                ...
                style="background: #f0f0ff; border: 1.5px solid var(--primary); color: var(--primary); padding: 6px 14px; border-radius: 4px; font-size: 0.85rem; cursor: pointer; font-weight: 600;">
...
<?php endif; ?>
        ...
        <div class="table-container">
            <h3 style="padding: 20px 25px; margin: 0; font-size: 1rem; font-weight:800; border-bottom: 1px solid #edf2f7; background: #fafbfc;">
                ...
                <tbody>
                    <?php foreach ($workEntries as $entry): ?>
                        <tr data-id="<?= $entry['id'] ?>">
                            ...
                            <td class="minutes">
                                <span style="background: #eef2ff; color: var(--primary); padding: 5px 12px; border-radius: 4px; font-weight: 700; font-family:monospace;">
                                    <?= $entry['minutes'] ?> min
                                </span>
                            </td>
    ...
    <div id="deleteHoursModal" class="modal">
        <div class="modal-content" style="max-width: 450px; text-align: center; padding: 30px;">
            ...
            <div style="margin-bottom: 20px;">
                ...
                <p style="color: #64748b; margin: 0; font-size: 0.95rem; line-height: 1.5;">Estás a punto de eliminar un registro de tiempo. Esta acción no se puede deshacer.</p>
                <p id="deleteHoursInfo" style="margin-top: 10px; font-weight: 600; background: #f1f5f9; padding: 8px; border-radius: 6px;"></p>
            </div>
            <div style="display: flex; gap: 10px; justify-content: center; margin-top: 25px;">
                <input type="hidden" id="deleteHoursId">
                <button type="button" onclick="closeDeleteHoursModal()" style="background: white; border: 1px solid #e2e8f0; color: #4a5568; padding: 12px 24px; border-radius: 8px; font-weight: 700; cursor: pointer; transition: 0.2s;">Cancelar</button>
                <button type="button" id="confirmDeleteHoursBtn" style="background: #dc2626; border: none; color: white; padding: 12px 24px; border-radius: 8px; font-weight: 700; cursor: pointer; transition: 0.2s; box-shadow: 0 4px 6px rgba(220, 38, 38, 0.2);">Sí, eliminar</button>
            </div>
        </div>
    </div>
...
searchInput.value = ''; // Limpiar buscador
    ...
    [clientSelect, serviceSelect].forEach(el => {
        el.style.backgroundColor = '#f0fff4'; // Color verde suave
